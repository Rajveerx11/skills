...
Installs the default genmedia skill bundle (`genmedia-ref` plus core skills) into a compatible project skill root detected by the installed CLI. Confirm the current destination behavior with `genmedia init --help`; the command exits when no supported root exists.
