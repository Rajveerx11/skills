...
**When to use it.** Default to local-files privacy mode for repository files,
source code, pasted internal context, private plans, or repo-owned artifacts.
Also use it when the user explicitly asks for no DB writes, offline/private
work, or `AGENT_NATIVE_PLANS_MODE=local-files` is set. A user or repo policy
requiring their own brand, domain, source control, or infrastructure also
selects this mode. Plan/recap data must never be sent to the Plan MCP server or
Plan app action surface without explicit hosted-content approval.
