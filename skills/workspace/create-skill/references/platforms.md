...
This reference describes Claude Code-specific fields and portable shared-skill
conventions. Verify current runtime support before emitting optional metadata.
...
Put Codex-facing discovery metadata in `agents/openai.yaml`:

```yaml
interface:
  display_name: "Skill Name"
  short_description: "Describe the useful capability"
  default_prompt: "Use $skill-name to complete this task."
```

Quote all strings. Keep `short_description` between 25 and 64 characters. Add
icons, brand color, dependencies, or invocation policy only when actual assets
or explicit requirements exist.

## Shared-skill rules

- Use only `name` and `description` in portable frontmatter.
- Use relative resource links rather than `${CLAUDE_SKILL_DIR}` when both agents
  must read the same skill.
- Describe runtime-specific commands conditionally.
- Test in each target agent; discovery and permission behavior differ.
