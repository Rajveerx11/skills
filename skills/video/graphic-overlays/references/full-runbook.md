# Graphic Overlays Full Runbook

Binding implementation detail for the `graphic-overlays` skill. Read this file before executing the pipeline; its exact media, seek, timing, HTML, and render contracts override generic video guidance.

## Contents

- [CLI resolution](#cli-resolution)
- [Workflow](#workflow)
  - [Check environment](#1-check-environment)
  - [Create a work directory](#2-create-a-work-directory)
  - [Extract audio and metadata](#3-extract-audio-and-metadata)
  - [Transcribe](#4-transcribe)
  - [Correct transcript](#5-correct-transcript)
  - [Draft storyboard](#6-draft-a-lightweight-storyboard-in-chat)
  - [Decide render strategy](#7-decide-render-strategy)
  - [Write card HTML](#8-write-each-cards-html)
  - [Assemble composition HTML](#9-assemble-the-composition-html)
  - [Render to MP4](#10-render-to-mp4)
  - [Report results](#11-report-results)
...
> **Confirm the route before you build.** This skill packages an **existing talking-head clip** with **designed graphic cards** (titles, lower-thirds, data callouts, quotes, side panels, PiP). If the user wants **plain captions / subtitles** (the spoken words as text) → `/embedded-captions`; a **single short unnarrated** element (one logo sting / lower-third) → `/motion-graphics`. **The clip plays untouched** — re-timing, recoloring, reframing, reordering, or audio is NLE editing and **out of scope**. Building from a URL / topic / PR → the creation workflows. Unsure overlays-vs-captions? **Use `/hyperframes` for the routing table first.**
...
1. **Native structured-input UI** — use the structured 4-question
   schema below through the host's normal user-input mechanism.
2. **No native structured-input UI** — **ask
...
**Channel A — native structured input (host-neutral pseudocode):**
  ...
  questions: [
    {
      ...
      header: "Aspect ratio",
      multiSelect: false,
      // Reorder so the recommended option appears FIRST.
...
**About custom counts** — if the host adds a free-form option, the user can type a number directly (e.g. "8", "20") as the cardCount target. Parse the input as an integer: if parsing succeeds → use that value (minimum 5 as a floor); if parsing fails → fall back to "auto".
...
2. **Map the style group to a specific style** by looking at the
   ...
   inside the group, ask one focused follow-up with those 2–4 specific
   style options through the host's normal input mechanism.
...
See
[`frames/`](frames/) for each frame's placement
