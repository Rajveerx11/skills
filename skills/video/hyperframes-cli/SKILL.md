...
### Windows gotchas (verified on real runs)

- **FFmpeg/FFprobe are NOT bundled — `render` hard-fails without them on PATH.** `doctor` shows `✗ FFmpeg`. If a Remotion project exists on the machine it ships a working build at `node_modules/@remotion/compositor-win32-x64-msvc/{ffmpeg,ffprobe}.exe` — prepend that dir to PATH for the render command: `$env:PATH = "<that dir>;$env:PATH"; npx hyperframes render ...`. Re-run `doctor` to confirm both go green before rendering.
- **`init` can fail silently (exit 9) or be blocked by a write-protected `C:\` root.** `init` is not required — `lint`, `inspect`, `validate`, and `render` operate on any directory containing an `index.html`. If `init` won't run, create the structure by hand: `<project>/index.html`, `<project>/public/{vo,sfx}/`, `<project>/renders/`.
- **Don't `cd` across drive roots in the bash tool** (`cd /c/ && …` resets the shell's cwd and aborts the command). Drive changes / project-dir work should go through PowerShell (`Set-Location`), or pass absolute paths and let the command's own cwd flag handle it.
- **Verify the render after it finishes**, not just the exit code: `ffprobe` the output for expected `width/height`, `duration`, and the presence of an `audio` (aac) stream; extract a hero frame per scene and eyeball it (catches issues lint/inspect can't, e.g. a tween that hides text).
