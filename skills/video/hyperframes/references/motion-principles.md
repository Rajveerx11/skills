...
- **`tl.fromTo()` that RE-animates an already-visible element needs `immediateRender: false`.** `fromTo` defaults to `immediateRender: true`, which writes the "from" state at timeline-construction time. For a first entrance that's fine (the element should be hidden until it enters). But when an element is already on screen earlier in the scene and you `fromTo` it again later — a counter digit that flips 1→2, a chip that re-pops, a label that swaps — the "from" (e.g. `opacity: 0`, `y: 22`) is applied from t=0, so the element is **invisible from the start until the later tween runs**. This lint-clean bug only shows in the render. Fix: add `immediateRender: false` to any `fromTo` whose target appears before the tween's start time.

  ```js
  // BAD: digit shows "1" should be visible from scene start, but fromTo hides it from t=0
  tl.to("#digit", { opacity: 0, y: -22, duration: 0.18 }, 13.5);            // fade old
  tl.call(() => (digit.textContent = "2"), null, 13.68);                     // swap
  tl.fromTo("#digit", { y: 22, opacity: 0 }, { y: 0, opacity: 1 }, 13.7);    // <-- hides it from t=0

  // GOOD: immediateRender:false keeps the element at its natural state until 13.7
  tl.fromTo("#digit", { y: 22, opacity: 0 }, { y: 0, opacity: 1, immediateRender: false }, 13.7);
  ```
