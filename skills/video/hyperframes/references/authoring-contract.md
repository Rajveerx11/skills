...
Binding implementation detail for the `hyperframes` skill. Read the relevant sections before authoring or changing a composition; exact composition, timeline, media, transition, and validation rules override generic video guidance.

## Contents

- [Approach](#approach)
- [Layout before animation](#layout-before-animation)
- [Data attributes](#data-attributes)
- [Composition structure](#composition-structure)
- [Variables](#variables-parametrized-compositions)
- [Video and audio](#video-and-audio)
- [Timeline contract](#timeline-contract)
- [Non-negotiable rules](#rules-non-negotiable)
- [Scene transitions](#scene-transitions-non-negotiable)
- [Animation guardrails](#animation-guardrails)
- [Typography and assets](#typography-and-assets)
...
2. **Want to browse options visually?** → Run the design picker: read [design-picker.md](design-picker.md) for the full workflow. This serves a visual picker page. The user configures mood, palette, typography, and motion in the browser, then copies the generated design.md and pastes it back into the conversation.
...
**design.md defines the brand. It does not define video composition rules.** Those come from [video-composition.md](video-composition.md) and [house-style.md](../house-style.md). Use brand colors at video-appropriate scale — not at web-UI opacity.
...
Read [prompt-expansion.md](prompt-expansion.md) for the full process and output format.
...
3. **Rhythm** — declare your scene rhythm before implementing. Which scenes are quick hits, which are holds, where do shaders land, where does energy peak. Name the pattern: fast-fast-SLOW-fast-SHADER-hold. Read [beat-direction.md](beat-direction.md) for rhythm templates.
...
- **[captions.md](captions.md)** — Captions, subtitles, lyrics, karaoke synced to audio. Tone-adaptive style detection, per-word styling, text overflow prevention, caption exit guarantees, word grouping. Read when adding any text synced to audio timing.
- **[audio-reactive.md](audio-reactive.md)** — Audio-reactive animation: map frequency bands and amplitude to GSAP properties. Read when visuals should respond to music, voice, or sound.
- **[css-patterns.md](css-patterns.md)** — CSS+GSAP marker highlighting: highlight, circle, burst, scribble, sketchout. Deterministic, fully seekable. Read when adding visual emphasis to text.
- **[video-composition.md](video-composition.md)** — Video-medium rules: density, color presence, scale, frame composition, design.md as brand not layout. **Always read** — these override web instincts.
- **[beat-direction.md](beat-direction.md)** — Beat planning: concept, mood, choreography verbs, rhythm templates, transition decisions, depth layers. **Always read for multi-scene compositions.**
- **[typography.md](typography.md)** — Typography: font pairing, OpenType features, dark-background adjustments, font discovery script. **Always read** — every composition has text.
- **[motion-principles.md](motion-principles.md)** — Motion design principles, image motion treatment, load-bearing GSAP rules. **Always read** — every composition has motion.
- **[techniques.md](techniques.md)** — 11 visual techniques with code patterns: SVG drawing, Canvas 2D, CSS 3D, kinetic type, Lottie, video compositing, typing effect, variable fonts, MotionPath, velocity transitions, audio-reactive. Read when planning techniques per beat.
- **[narration.md](narration.md)** — Pacing, tone, script structure, number pronunciation, opening line patterns. Read when the composition includes voiceover or TTS.
- **[design-picker.md](design-picker.md)** — Create a design.md via visual picker. Read when no design.md exists and the user wants to create one.
...
- **[transcript-guide.md](transcript-guide.md)** — Caption-side transcript handling: input formats, mandatory quality check, cleaning JS, OpenAI/Groq API fallback, "if no transcript exists" flow. (For the `transcribe` CLI invocation, model selection rules, and the `.en` gotcha, see the `hyperframes-media` skill.)
- **[dynamic-techniques.md](dynamic-techniques.md)** — Dynamic caption animation techniques (karaoke, clip-path, slam, scatter, elastic, 3D).

- **[transitions.md](transitions.md)** — Scene transitions: crossfades, wipes, reveals, shader transitions. Energy/mood selection, CSS vs WebGL guidance. **Always read for multi-scene compositions** — scenes without transitions feel like jump cuts.
  - [transitions/catalog.md](transitions/catalog.md) — Hard rules, scene template, and routing to per-type implementation code.
