...
function main() {
  ...
  if (!checkPlaywrightInstalled()) {
    ...
    console.error(`Run "npm run setup" in ${__dirname} after dependency installation is authorized.`);
    process.exit(1);
  }
  ...
  try {
    ...
    // Execute in a child process so completion and cleanup are deterministic.
    console.log('🚀 Starting automation...\n');
    const child = spawnSync(process.execPath, [tempFile], {
      cwd: __dirname,
      env: {
        ...process.env,
        PLAYWRIGHT_SKILL_DIR: __dirname,
      },
      stdio: 'inherit',
    });
    if (child.error) {
    ...
    try {
      ...
      if (error.code !== 'ENOENT') {
        console.warn(`⚠️ Could not remove temporary executor file: ${error.message}`);
      }
    }
  }
}
